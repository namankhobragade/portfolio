    ...
    const logVisitor = async () => {
      ...
      if (clientInfo) {
        const { error } = await supabase.from('visitors').insert({
          ...
          screen_resolution: clientInfo.screen ? `${clientInfo.screen.width}x${clientInfo.screen.height} @${clientInfo.screen.pixelRatio}x (${clientInfo.screen.colorDepth}-bit)` : undefined,
